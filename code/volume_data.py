...
volumes = pd.read_csv('../temporary/volumes.csv')
industry = pd.read_csv('../input/industry_scores_jan2025.csv')
progress = pd.read_csv('../input/progress_scores_jan2025.csv')
...
industry = industry.rename(columns={'Unnamed: 0': 'HTID', 'Industrial Scores (June 23)': 'industry_score'})
industry['HTID'] = industry['HTID'].map(lambda x: x.rstrip('.txt'))#remove '.txt' at the end of each string for HTIDs

progress = progress.rename(columns={'Unnamed: 0': 'HTID', 'Progress': 'progress_score'})
progress['HTID'] = progress['HTID'].map(lambda x: x.rstrip('.txt'))#remove '.txt' at the end of each string for HTIDs

dfs = [metadata, volumes, industry, progress]
...
print('Progress Dimensions:' + str(progress.shape))
print('Merged Dimensions:' + str(volumes_scores.shape))

print(volumes_scores.head())

volumes_scores['industry_percentile'] = volumes_scores.industry_score.rank(pct=True, method = 'min')
volumes_scores['progress_percentile_main'] = volumes_scores.progress_score.rank(pct=True, method = 'min')
