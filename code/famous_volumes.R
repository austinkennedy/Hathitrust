...
plot <- ggtern(famous_selected, aes(x = Political.Economy, y = Religion, z = Science, shape=Title, color = Title)) +
  ...
  scale_shape_manual(values = famous_selected$shape, labels = famous_selected$Title) +
  scale_color_manual(values = famous_selected$color, labels = famous_selected$Title) +
  ...
  theme(tern.axis.title.R = element_text(hjust=0.6, vjust = 0.9, size = 10), tern.axis.title.L = element_text(hjust = 0.3, vjust = 0.9, size = 10),
        ...
        legend.title = element_text(size = 14, face = 'bold'), legend.text = element_text(size = 10), legend.spacing.y = unit(0.1, 'cm')) +
  guides(color = guide_legend(byrow =TRUE, size =10),
         shape = guide_legend(byrow=TRUE, size =10))
